...
<div class="container-fluid bg-primary px-5 d-none d-lg-block">
    <div class="row gx-0 align-items-center">
        <div class="col-lg-5 text-center text-lg-start mb-lg-0">
            <div class="d-flex">
                <a href="#" class="text-white me-4"><i class="fas fa-envelope text-light me-2"></i>admin@layanansurat.com</a>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-white px-4 px-lg-5 py-3 py-lg-0">
        <a href="{{ url('/') }}" class="navbar-brand p-0">
            <h1 class="display-5 text-secondary m-0">
                <img src="{{ asset('assets-guest/img/logo-website.png') }}" class="img-fluid" alt="Logo">
            </h1>
        </a>
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto py-0">
                ...
                {{-- Menu Data Warga --}}
                <a href="{{ route('warga.index') }}" class="nav-item nav-link {{ request()->routeIs('warga.index') ? 'active' : '' }}">
                    <i class="fas fa-users me-1"></i>Data Warga
                </a>

                {{--
                    MENU MASTER DATA (BARU)
                    Hanya tampil jika user login DAN role-nya super_admin atau staff
                --}}
                @if(Auth::check() && in_array(Auth::user()->role, ['super_admin', 'staff']))
                    <div class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle {{ request()->routeIs('jenis-surat.*', 'berkas.*', 'riwayat.*') ? 'active' : '' }}" data-bs-toggle="dropdown">
                            <i class="fas fa-database me-1"></i>Master Data
                        </a>
                        <div class="dropdown-menu m-0">
                            <a href="{{ route('jenis-surat.index') }}" class="dropdown-item">
                                <i class="fas fa-envelope-open-text me-2"></i>Jenis Surat
                            </a>
                            <a href="{{ route('berkas.index') }}" class="dropdown-item">
                                <i class="fas fa-clipboard-list me-2"></i>Berkas Persyaratan
                            </a>
                            <a href="{{ route('riwayat.index') }}" class="dropdown-item">
                                <i class="fas fa-history me-2"></i>Log Riwayat
                            </a>
                        </div>
                    </div>
                @else
                    {{-- Jika Guest/Warga biasa, tampilkan Jenis Surat sebagai menu biasa --}}
                    <a href="{{ route('jenis-surat.index') }}" class="nav-item nav-link {{ request()->routeIs('jenis-surat.index') ? 'active' : '' }}">
                        <i class="fas fa-envelope me-1"></i>Jenis Surat
                    </a>
                @endif

                {{-- Menu Permohonan Surat --}}
                <a href="{{ route('permohonan-surat.index') }}" class="nav-item nav-link {{ request()->routeIs('permohonan-surat.index') ? 'active' : '' }}">
                    <i class="fas fa-file-import me-1"></i>Permohonan Surat
                </a>

                {{-- Dropdown Layanan --}}
                <div class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle {{ request()->is('feature', 'countries', 'testimonial') ? 'active' : '' }}" data-bs-toggle="dropdown">
                        <i class="fas fa-concierge-bell me-1"></i><span class="dropdown-toggle">Layanan</span>
                    </a>
                    <div class="dropdown-menu m-0">
                        <a href="{{ route('permohonan-surat.create') }}" class="dropdown-item">
                            <i class="fas fa-file-import me-2"></i>Ajukan Permohonan
                        </a>
                        <a href="{{ route('permohonan-surat.index') }}" class="dropdown-item">
                            <i class="fas fa-tasks me-2"></i>Status Permohonan
                        </a>
                ...
                {{-- Menu Data User (Biasanya untuk Admin) --}}
            ...
            {{-- Tombol Ajukan Surat (CTA) --}}
            <a href="{{ route('permohonan-surat.create') }}" class="btn btn-primary border-secondary rounded-pill py-2 px-4 px-lg-3 mb-3 mb-md-3 mb-lg-0">
